...
writes TestMessages.r1st_success, TestMessages.r1st_complete {
	...
	@thread
	@generated("blockdiagram", "846bdf25")
	public void calc() {
		...
		EdgeRising_instance.compute(^start); // Main/calc 4
		if (EdgeFalling_instance.value()) {
			TestMessages.r1st_success = false; // Main/calc 5/if-then 1
			TestMessages.r1st_complete = false; // Main/calc 5/if-then 2
			result = true; // Main/calc 5/if-then 3
		} // Main/calc 5
		if (EdgeRising_instance.value()) {
			Timer_instance_2.begin(5.0); // Main/calc 6/if-then 1
			Timer_instance.begin(15.0); // Main/calc 6/if-then 2
		} // Main/calc 6
		if (^start) {
			Timer_instance_2.compute(); // Main/calc 7/if-then 1
			Timer_instance.compute(); // Main/calc 7/if-then 2
		} // Main/calc 7
	}
}